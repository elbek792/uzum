...
from .models import Category, Product, Comment, Registration
from .forms import LogingForm,CategoriForms,ProdactForms, ProductImageForm, EditForm, RegistrationForm
...
def detail(request, id):
	...
	if request.POST.get('izoh'):
		...
		return redirect(reverse('detail', args=[a.id]) + f"#{name}")
	return render(request, 'single-post.html', {'a':a, 'similar_products' : similar_products,  'category' : Category.objects.all()})
